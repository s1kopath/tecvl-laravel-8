...
class BlogController extends Controller
{
    ...
    public function index(BlogDataTable $dataTable)
    {
        ...
        $data['auhtor_id'] = !empty(request()->input("auhtor_id")) ? request()->input("auhtor_id") : '';
        $data['blog'] = (new Blog())->getAllBlogDT($data['from'], $data['to'], $data['category_id'], $data['auhtor_id']);
        $data['categoaries'] = BlogCategory::where('status', 'Active')->get();
        $data['authors'] = User::select('id', 'name')->where('status', 'Active')->get();
    ...
    public function delete($id)
    {
        if ($blog = Blog::find($id)) {
            try {
                ...
                DB::rollBack();
                return redirect()->route('blog.index')->withFaile(__('Something went wrong.'));
            }
        } else {
            return redirect()->route('blog.index')->withFaile(__('Blog does not found.'));
        }
    }
}
