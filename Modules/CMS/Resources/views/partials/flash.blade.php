...
<div class="w-322p">
    <div class="relative bg-gray-11 rounded-md pb-0.5">
        @isset($data['badge_text'])
        <div class="p-4">
            <p class="text-xs rounded-sm roboto-medium font-medium text-gray-12 text-center px-1.5 py-1 bg-yellow-1 w-25">
            {{ $data['badge_text'] }}</p>
        </div>
        @endisset
        <div class="flex justify-center">
            <img width="180px" height="160px" src="{{ $flashProduct->fileUrl(['home' => true]) }}" alt="">
        </div>
        ...
        <div class="text-center bg-white mx-4 mb-4 mt-8 py-4 rounded">
                                ...
                                <li class="mt-1 pr-2">
                                    <svg class="h-3 w-3" viewBox="0 0 142 142" fill="none"
                                        ...
                                        <path
                                            ...
                                            fill="#C4C4C4" />
                                        <mask id="mask0_2170_1814" style="mask-type:alpha" maskUnits="userSpaceOnUse"
            ...
            @if ($flashProduct->isDiscountable())
                <p class="text-xl text-gray-12 dm-bold mt-3 pb-4">
                    {{ $currency_symbol }}{{ formatCurrencyAmount($flashProduct->discounted_price) }}
                </p>
            @endif
            <p
                class="{{ $flashProduct->isDiscountable()? 'text-12 font-medium line-through text-gray-10 pl-1 mt-0.5': 'text-xl text-gray-12 dm-bold mt-3 pb-4' }}">
                {{ $currency_symbol }}{{ formatCurrencyAmount($flashProduct->price) }}
            </p>
        </div>
    </div>
</div>
