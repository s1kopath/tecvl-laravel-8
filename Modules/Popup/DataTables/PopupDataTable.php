...
class PopupDataTable extends DataTable
{
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('name', function ($popup) {
                return  wrapIt($popup->name, 10, ['columns' => 1]);
            })->addColumn('show_time', function ($popup) {
            ...
            })->addColumn('start_date', function ($popup) {
                return !empty($popup->start_date) ? formatDate($popup->start_date) : '';
            })->addColumn('end_date', function ($popup) {
                ...
                $delete = '<form method="post" action="' . route('popup.delete', ['id' => $popup->id]) . '" id="delete-popup-'. $popup->id . '" accept-charset="UTF-8" class="display_inline">
                        ' . csrf_field() . '
                        <button title="' . __('Delete :?', ['?' => __('popup')]) . '" class="btn btn-xs btn-danger" type="button" data-id=' . $popup->id . ' data-label="Delete" data-delete="popup" data-toggle="modal" data-target="#confirmDelete" data-title="' . __('Delete :?', ['?' => __('popup')]) . '" data-message="' . __('Are you sure to delete this?') . '">
