        ...
        return datatables()
            ...
            ->editColumn('vendor.name', function ($shops) {
                ...
                $delete = '<form method="post" action="' . route('shop.destroy', ['id' => $shops->id]) . '" id="delete-shops-'. $shops->id . '" accept-charset="UTF-8" class="display_inline">
                        ' . csrf_field() . '
                        <button title="' . __('Delete :x', ['x' => __('Shop')]) . '" class="btn btn-xs btn-danger" type="button" data-id=' . $shops->id . ' data-label="Delete" data-delete="shops" data-toggle="modal" data-target="#confirmDelete" data-title="' . __('Delete :x', ['x' => __('Shop')]) . '" data-message="' . __('Are you sure to delete this?') . '">
