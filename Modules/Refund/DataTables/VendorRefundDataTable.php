    ...
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->filter(function ($instance){
                if(in_array(request('status'), ['Pending', 'Accepted', 'Rejected', 'Confirmed'])) {
                ...
                if (isset(request('search')['value'])) {
                    ...
                    if (!empty($keyword)) {
                        $instance->where(function ($query) use ($keyword) {
                            $query->WhereLike('shipping_method', $keyword)
                                ->OrWhereLike('refund_method', $keyword)
                                ->OrWhere('quantity_sent', $keyword)
                                ->OrWhereLike('status', $keyword)
                                ->orWhereHas('orderDetail.order', function ($query)use($keyword) {
                                    $query->WhereLike('reference', $keyword);
                                })
                                ->orWhereHas('orderDetail', function ($query)use($keyword) {
                                    $query->WhereLike('price', $keyword);
                                })
                                ->orWhereHas('refundReason', function ($query)use($keyword) {
