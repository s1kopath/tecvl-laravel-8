...
class MenuItems extends Model
{
    ...
    {
        if ($this->is_custom_menu == 1 && !empty($this->link)) {
                return $this->link;
        } else {
     ...
     * @return boolean
     */
    public function hasPermission($prms)
    {
        if ($this->is_custom_menu == 1) {
            return true;
        }

        if (!$this->isParent()) {
            return in_array($this->permission, $prms);
        }

        foreach ($this->child as $child) {
            if (in_array($child->permission, $prms)) {
                return true;
            }
        }
        return false;
