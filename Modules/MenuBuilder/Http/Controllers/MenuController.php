...
class MenuController extends Controller
{
    ...
    */
    public function deleteMenu()
    {
        ...
        if (!empty($menuitem)) {
            $checkDuplicate = MenuItems::where('id', '!=', request()->input("id"))->where(['link' => $menuitem->link, 'menu' => $menuitem->menu])->first();
            if ($menuitem->is_default == 1 || $menuitem->menu == 4) {
                if (!empty($checkDuplicate) || $menuitem->menu == 4) {
                    $menuitem->delete();
                    return json_encode(["resp" => __("Deleted Successfully"), "error" => 1]);
                } else {
                    return json_encode(["resp" => __("You can't delete this menu"), "error" => 2]);
                }
            } else if ($menuitem->is_default == 0) {
                $menuitem->delete();
                return json_encode(["resp" => __("Deleted Successfully"), "error" => 1]);
            }
        } else {
    ...
    */
    ...
    {
        Menus::where('id', request()->input("idmenu"))->update(['name' => request()->input("menuname")]);
