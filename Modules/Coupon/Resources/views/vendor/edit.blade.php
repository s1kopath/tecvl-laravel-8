...
@section('content')
    @php
        ...
        foreach ($coupon->items as $item) {
            array_push($couponItem, $item->id);
        }
    @endphp
    <div class="col-sm-12" id="vendor-coupon-edit-container">
        <div class="card">
            <div class="card-body">
                                                ...
                                                <div class="col-sm-6">
                                                    <select class="form-control select2 sl_common_bx" id="item_id" name="item_ids[]" multiple>
                                                        @foreach ($items as $item)
                                                            <option {{ in_array($item->id, $couponItem) ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
