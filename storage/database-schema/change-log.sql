...
(
    ...
    KEY           `sms_templates_name_index` (`name`),
    KEY           `sms_templates_status_index` (`status`),
    KEY           `sms_templates_language_id_foreign_idx` (`language_id`),
    KEY           `sms_templates_parent_id_foreign_idx` (`parent_id`),
    CONSTRAINT `sms_templates_language_id_foreign` FOREIGN KEY (`language_id`) REFERENCES `languages` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `sms_templates_parent_id_foreign` FOREIGN KEY (`parent_id`) REFERENCES `sms_templates` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- database schema for create table packages
CREATE TABLE `packages` (
  ...
  `price` decimal(16,8) DEFAULT NULL,
  `billing_cycle` varchar(45) DEFAULT NULL COMMENT 'Monthly / Yearly',
...
-- database schema for create table package_subscriptions
CREATE TABLE `package_subscriptions` (
  ...
  `billing_price` decimal(16,8) DEFAULT '0.00',
  `billing_cycle` varchar(45) DEFAULT NULL,
  `amount_billed` decimal(16,8) DEFAULT '0.00',
  `amount_received` decimal(16,8) DEFAULT '0.00',
  `amount_due` decimal(16,8) DEFAULT '0.00',
  `payment_processor` varchar(45) DEFAULT '2co',
  `transaction_order_number` varchar(45) DEFAULT NULL,
  `transaction_invoice_id` varchar(45) DEFAULT NULL,
  `transaction_reference` varchar(45) DEFAULT NULL,
  `is_customized` tinyint(4) DEFAULT NULL,
  `customized_records` int(11) DEFAULT NULL,
  `is_renewed` tinyint(1) DEFAULT NULL,
  `status` varchar(45) DEFAULT NULL COMMENT 'Pending / Active / Expired / Paused',
  `created_at` timestamp    NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `package_subscriptions_vendor_id_index` (`vendor_id`),
  KEY `package_subscriptions_package_id_index` (`package_id`),
  KEY `package_subscriptions_status_index` (`status`),
  KEY `package_subscriptions_payment_processor_index` (`payment_processor`),
  KEY `package_subscriptions_transaction_reference_index` (`transcation_reference`),
  KEY `package_subscriptions_transaction_order_number_index` (`transcation_order_number`),
  KEY `package_subscriptions_transaction_invoice_id_index` (`transcation_invoice_id`),
  KEY `package_subscriptions_billing_date_index` (`billing_date`),
  KEY `package_subscriptions_next_billing_date_index` (`next_billing_date`),
  KEY `package_subscriptions_billing_email_index` (`billing_email`),
  KEY `package_subscriptions_is_renewed_index` (`is_renewed`),
  KEY `package_subscriptions_is_customized_index` (`is_customized`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
