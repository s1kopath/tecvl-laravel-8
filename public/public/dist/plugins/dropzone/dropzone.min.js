	...
	$(document).on('click', selector, function() {
		...
		.then((willDelete) => {
		  	if (willDelete) {
                ...
                var thumbnail = typeof(element.attr('data-thumbnail')) != 'undefined' ? element.attr('data-thumbnail') : null;
                    $.ajax({
		          	type: "POST",
		          	url: url,
		          	data: {
		          	...
		          	success: function(response) {
			            if (response.status == 1) {
			            	if ($('#table_id_' + $(selector).attr('data-id')).length) {
			            		$('#table_id_' + $(selector).attr('data-id')).remove();
			            	} else {
			            		parent.remove();
			            	}
			            } else {
	    ...
	    success: function(file, response) {
	      	...
	      	if (res.result == true) {
	      		if (dataTableUpdate == true) {
	        		...
	        		$('.add-attachments').before('<div class="list-attachments"><i class="fa fa-times attachment-item-delete" data-id="0" data-attachment="' + res.data.attachment_path + '" title="Delete Attachment" aria-hidden="true"></i><div><i class="'+ res.data.attachment_type +'" title="'+ res.data.attachment_name +'"></i><input type="hidden" name="attachments[]" value="' + res.data.attachment + '"></div></div>');
	      		}
	        	msg = "";
	      	} else {
...
function uploadAttachmentById(url, selectorId, attachments, selector, count, dataTableUpdate = false) {
    var statusPicture = $(selectorId);
    $(selector).dropzone({
        url: url,
        paramName: "attachment",
        headers: {
