...
$(document).on('click', ".btn-file-add", function() {
    let extension = $(this).attr('data-info');
    let fileSize = Number($(this).attr('data-size'))/1024;
    
    let errorMsg = jsLang('File extension must be :x and :y MB.');
    
     if ($(this).attr('dataFavicon') == 'icon') {
        if ( extension != 'ico' || fileSize > imageSize) {
               $("#company_favicon").removeClass("preview-image");
                $('#company_favicon').html('');
                errorMsg = errorMsg.replace(":x", 'ico');
                errorMsg = errorMsg.replace(":y", imageSize);
                $('#company_favicon').html('<span class="text-red">' + errorMsg +'</span>');
            } else {
                $('#company_favicon').addClass("preview-image");
                $('#company_favicon').html('');
            }
     } else if ($(this).attr('dataFavicon')=='logo') {
         let extensions = ['jpg', 'jpeg', 'png'];
         if (extensions.includes(extension) && (fileSize <= imageSize)) {
                $('#company_logo').addClass("preview-image");
                $('#company_logo').html('');
         } else {
                $("#company_logo").removeClass("preview-image");
                $('#company_logo').html('');
                errorMsg = errorMsg.replace(":x", extensions.join(', '));
                errorMsg = errorMsg.replace(":y", imageSize);
                $('#company_logo').html('<span class="text-red">' + errorMsg +'</span>');
        }
     }
})
