...
class AttributeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            DB::beginTransaction();
            $attributeId = (new Attribute)->store($request->only('name', 'type', 'attribute_group_id', 'is_filterable', 'is_required', 'status', 'description'));
            if (!empty($attributeId)) {
                ...
                foreach ($request->category_ids as $categoryId) {
                    (new CategoryAttribute)->store(['attribute_id' => $attributeId, 'category_id' => $categoryId]);
                }
                DB::commit();
    ...
    public function edit($id)
    {
        ...
        $data['categories'] = (new Category)->parents();
        $data['allCategoryAttributes'] = CategoryAttribute::where('attribute_id', $id)->pluck('category_id')->toArray();
    ...
    {
        $response = $this->messageArray(__('Invalid Request'), 'fail');
        $result = $this->checkExistance($id, 'attributes');
        if ($result['status'] === true) {
            ...
            try {
                ...
                if ((new Attribute)->updateAttribute($request->only('name', 'type', 'attribute_group_id', 'category_id', 'is_filterable', 'is_required', 'status', 'description'), $id)) {
                    ...
                    $categoryAttributes = CategoryAttribute::where('attribute_id', $id)->pluck('category_id')->toArray();
                    $deletedCategory = array_diff($categoryAttributes, $request->category_ids);
                    $attributeValue = $request->values;
                    ...
                    foreach ($request->category_ids as $categoryId) {
                        if (is_array($categoryAttributes) && !in_array($categoryId, $categoryAttributes)) {
                            (new CategoryAttribute)->store(['attribute_id' => $id, 'category_id' => $categoryId]);
                        }
                    }
                    foreach ($deletedCategory as $cat) {
                        (new CategoryAttribute)->remove($id, $cat);
                    }
                    DB::commit();
                    $response = $this->messageArray(__('The :x has been successfully saved.', ['x' => __('Attribute')]), 'success');
                }
