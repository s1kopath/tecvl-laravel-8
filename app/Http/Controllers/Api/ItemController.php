...
class ItemController extends Controller
{
    ...
    {
        $id = $request->item_id;
        $response = $this->checkExistance($id, 'items');
        if ($response['status']) {
            if ($type == 'relate') {
                return $this->response([
                    'data' => (new Item)->search($request->search, $request->item_id, $type, "api"),
                ]);
            }
            if ($type == 'cross') {
                return $this->response([
    ...
    public function detail($id)
    {
        ...
        if ($response['status']) {
            return $this->response([
                'data' => new ItemDetailResource(Item::where('id', $id)->first())
            ]);
        }
        return $this->unprocessableResponse($response['message']);
    }
    ...
    {
        $response = $this->checkExistance($request->item_id, 'items');
        if ($response['status']) {
            $data = ItemOption::where('item_id', $request->item_id)->orderBy('order_by', 'ASC')->get();
            $optionData = [];
            foreach ($data as $dt) {
                $optionData[] = [
                    "name" => $dt->name,
                    ...
                    "values" => json_decode($dt->payloads)
              ];
            }
            return $this->response([
                'data' => $optionData,
    ...
    {
        $result = $this->checkExistance($request->item_id, 'items');
        if ($result['status'] === true) {
            ...
            return $this->unprocessableResponse($result['message']);
        }
    }
