...
use App\Http\Resources\{TopSellerResource, UserAddressesResource, userDetailResource};
use App\Models\{Address, Item, Order, OrderStatus, User};
    ...
    public function track(Request $request)
    {
        ...
        foreach ($orderStatus as $status) {
                ...
                $data[] = [
                    ...
                    'orderDate' => !empty($orderDate) && $active == true ? timeZonegetTime($orderDate->created_at) ." ". timeZoneformatDate($orderDate->created_at) : ''
                ];
        }
        if (!empty($order)) {
            return $this->response([
        ...
        return $this->notFoundResponse('', __('Record not found'));
    }
}
