...
class CategoryController extends Controller
{
    ...
    public function index(Request $request, $param = null)
    {
        ...
        if ($param == 'top') {
            $category = Category::whereHas('items', function(Builder $query) {
                $query->whereNotNull('purchase_count');
            })->withSum('items', 'purchase_count')->orderByDesc('items_sum_purchase_count');
        } else {
    ...
    {
        $configs = $this->initialize([], $request->all());
        $response = $this->checkExistance($parentId, 'categories');
        if ($response['status']) {
