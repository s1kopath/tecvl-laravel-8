...
class OrderStatusController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            DB::beginTransaction();
            $statusId = (new OrderStatus)->store($request->only('name', 'is_default', 'order_by'));
            if (!empty($statusId)) {
                $roleData = [];
    ...
    public function update(Request $request)
    {
        ...
        if ((new OrderStatus)->statusUpdate($request->only('name', 'is_default', 'order_by'), $request->status_id)) {
            $response = $this->messageArray(__('The :x has been successfully saved.', ['x' => __('Order Status')]), 'success');
