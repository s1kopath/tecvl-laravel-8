...
class CategoryController extends Controller
{
    ...
    {
        $categories = Category::getAll()->whereNull('parent_id')->sortBy('order_by')->all();
        $categoriesChild = Category::getAll()->whereNotNull('parent_id')->sortBy('order_by')->all();
        $data = [];
        $children = [];
        $subChildren = [];
        foreach($categoriesChild as $child) {
            $subChilds = Category::getAll()->where('parent_id', $child->id)->sortBy('order_by')->all();
    ...
    public function destroy(Request $request)
    {
        ...
        $id = $request->id;
        if ($request->isMethod('post')) {
            $result = $this->checkExistance($id, 'categories');
