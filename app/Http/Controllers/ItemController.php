...
class ItemController extends Controller
{
    ...
    public function index(ItemDataTable $dataTable)
    {
        ...
        $data['vendors'] = Item::select('vendor_id')->distinct()->with('vendor:id,name')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (isActive('Shop')) {
                $shopDetails = Shop::where('vendor_id', $request->vendor_id ?? null)->where('status', 'Active')->first();
    ...
    public function view($id)
    {
        ...
        $data['itemSeo'] = Seo::where('item_id', $id)->first();
    ...
    {
        if ($type == 'relate') {
            (new Item)->search($request->search, $request->item_id, $type);
        }
        if ($type == 'cross') {
            (new Item)->search($request->search, $request->item_id, $type);
        }
    ...
    {
        $response = $this->messageArray(__('Invalid Request'), 'fail');
        $result = $this->checkExistance($id, 'items');
    ...
    public function updateRealted($type, Request $request)
    {
        $response = $this->messageArray(__('Invalid Request'), 'fail');
        $data = [];
        $existValue = [];
        ...
        $this->setSessionValue($response);
        return redirect()->route('item.view',$request->item_id);
    }
        ...
        foreach ($itemOption as $option) {
            ...
            if (isset($payloads->inventory_id)) {
                foreach ($payloads->inventory_id as $id) {
                    ...
                    $inventoryData[$id] = $inventory->quantity ?? 0;
                }
            }
            $itemAllOption[] = [
                'id' => $option->id,
    ...
    public function csv()
    {
        return Excel::download(new ItemListExport(), 'item_lists' . time() . '.csv');
    }
    ...
    public function import(Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            return view('admin.items.import', $data);
            ...
            if ($request->hasFile('file')) {
                ...
                foreach ($csv as $key => $value) {
                    ...
                    $value['available_from'] = DbDateFormat($value['available_from']);
                    $value['available_to'] = DbDateFormat($value['available_to']);
                    $value['discount_from'] = DbDateFormat($value['discount_from']);
                    $value['discount_to'] = DbDateFormat($value['discount_to']);
                    ...
                    if (!empty($value['tags']) && is_null(json_decode($value['tags']))) {
                        $errorFails[] = __('The :attribute format is invalid.', ['attribute' => __('Tags')]);
                    }
                    if (!empty($value['attribute']) && is_null(json_decode($value['attribute']))) {
                        $errorFails[] = __('The :attribute format is invalid.', ['attribute' => __('Attribute')]);
                    }
                    if (!empty($value['option']) && is_null(json_decode($value['option']))) {
                        $errorFails[] = __('The :attribute format is invalid.', ['attribute' => __('Option')]);
                    }
                        ...
                        try {
                            ...
                            if (!empty($lastItemId)) {
                                ...
                                if (!empty($options)) {
                                    ...
                                    (new ItemOption)->import($optionData);
                                }
    ...
    public function seoStoreOrUpdate(Request $request)
    {
        $response = $this->messageArray(__('Invalid Request'), 'fail');
        if ((new Seo)->storeOrUpdate($request->only('item_id', 'title', 'description', 'slug'), 'item_id')) {
            $response = $this->messageArray(__('The :x has been successfully saved.', ['x' => __('SEO')]), 'success');
        }
        $this->setSessionValue($response);
        return redirect()->route('item.view',$request->item_id);
    }
