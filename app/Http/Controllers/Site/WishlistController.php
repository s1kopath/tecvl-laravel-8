...
class WishlistController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (isset($request->filter_day) && array_key_exists($request->filter_day, $filterDay)) {
            $wishlist = Auth::user()->wishlist()->whereDate('created_at', '>=', $filterDay[$request->filter_day]);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ((new Wishlist)->checkExistence($request['user_id'], $request->item_id)) {
            if (!isset($request->store_only)) {
                if(Wishlist::where('user_id', \Auth::user()->id)->where('item_id', $request->item_id)->delete()) {
    ...
    {
        if (Wishlist::where('user_id', \Auth::user()->id)->where('item_id', $itemId)->delete()) {
