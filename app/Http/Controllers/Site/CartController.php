...
use App\Models\{Inventory, Item, ItemOption, Vendor};
...
class CartController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($cartsValue as $key => $val) {
            ...
            if (isActive('Shipping')) {
                if (isset($item->itemDetail->shipping)) {
                    $priceWithQty = $val['price'] * $val['quantity'];
                    if (empty($item->itemDetail->shipping->minimum_amount) || $priceWithQty >= $item->itemDetail->shipping->minimum_amount) {
                        $shipping = $item->itemDetail->shipping->cost;
                    }
                }
            }

            if (isset($item->itemDetail->tax->tax_rate)) {
                $tax = $item->itemDetail->tax->tax_rate;
            }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($item) && $availability == 1) {
            ...
            if (isset($request->cartIndex) && !is_null($request->cartIndex)) {
                ...
                $optionLabel = isset($request->options) && count($request->options) > 0 ? $request->options : [];
                $optionNoLabel = isset($request->optionNoLabel) && count($request->optionNoLabel) > 0 ? $request->optionNoLabel : [];
                ...
                //no label
                if (isset($request->optionNoLabel) && is_array($request->optionNoLabel) && count($request->optionNoLabel) > 0) {
                    foreach ($request->optionNoLabel as $key => $opNoLabel) {
            ...
            if (count($optionCheck) == 0) {
                if ($inventoryEnable == true) {
                    $add = Cart::add(
                        [
                            ...
                            'shop_id' => isset($item->vendor->shops[0]->id) ? $item->vendor->shops[0]->id : null,
                            'name' => $item->name,
                            'quantity' => $request->qty ?? 1,
                            'price' => $price != null && $price > 0 ? $price : 0,
                            'actual_price' => $item->price,
                            'photo' => $item->fileUrl(),
                            'discount_amount' => $item->isDiscountable() ? $item->discount_amount : 0,
                            'discount_type' => $item->isDiscountable() ? $item->discount_type : null,
                ...
                if (count($txt) == 1) {
                    $response['message'] = __(':x is required.', ['x' => implode(",", $txt)]);
                } else {
                    $response['message'] = __(':x are required.', ['x' => implode(",", $txt)]);
                }
            }
        }
        return $response;
    ...
    public function checkCoupon(Request $request)
    {
        ...
        if (isActive('Coupon')) {
            if (!empty(Cart::getSelected())) {
                ...
                $response['message'] = __('Coupon already applied');
            } else {
                $response['message'] = __('Select an item first!');
            }
        } else {
    ...
    public function destroySelected(Request $request)
    {
        $response['status'] = 0;
        $response['message'] =  __("Something went wrong, please try again.");
        foreach ($request->item_id as $index) {
            Cart::destroy($index);
        }
        $response = [
            "status" => 1,
    ...
    public function storeSelected(Request $request)
    {
        $response['status'] = 0;
        $response['message'] =  __("Something went wrong, please try again.");
        Cart::selectedStore($request->item_id);
        $response = [
            "status" => 1,
            "message" => __("Saved Successfully."),
        ];
