...
class VendorOrderListExport implements FromCollection,WithHeadings, WithMapping
{
    ...
    public function map($orderList): array
    {
        return[
            ...
            formatCurrencyAmount($orderList->getTotalVendorItem(session()->get('vendorId'), $orderList->id)),
            formatCurrencyAmount($orderList->vendorItemPrice(session()->get('vendorId'), $orderList->id)),
            $orderList->status,
