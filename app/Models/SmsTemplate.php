...
class SmsTemplate extends Model
{
    ...
    protected static function storeValidation($data = [])
    {
        $validator = Validator::make($data, [
            ...
            'slug' => 'required|unique:sms_templates,slug',
            'body' => 'required',
    ...
    {
        $id = parent::insertGetId(array_intersect_key($data, array_flip((array)['name', 'slug', 'body', 'status', 'language_id', 'variables'])));

        if (isset($id) && !empty($id)) {
            if (isset($data['data']) && !empty($data['data'])) {
                foreach ($data['data'] as $key => $value) {
                    if (!empty($value['body']) && !empty($value['language_id'])) {
                        $value['parent_id'] = $id;
                        parent::insert($value);
                    }
                }
    ...
    public function remove($id = null)
    {
        ...
        if (!empty($record)) {
            try {
                ...
                $record->delete();
                $data['status']  = 'success';
                $data['message'] = __('The :x has been successfully deleted.', ['x' => __('SMS Template')]);
            } catch (Exception $e) {
    ...
    public function updateTemplate($request = [], $id = null, $exist = null)
    {
        $data = ['status' => 'fail', 'message' => __('Something went wrong, please try again.')];
        try {
            $array = ['name', 'body', 'status', 'language_id', 'variables'];
            ...
            self::forgetCache();
            $data['status']  = 'success';
            $data['message'] = __('The :x has been successfully saved.', ['x' => __('SMS template')]);
        } catch (Exception $e) {
            $data['message'] = $e->getMessage();
        }
