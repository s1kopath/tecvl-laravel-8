...
class Attribute extends Model
{
    ...
    protected static function storeValidation($data = [])
    {
        $rules = [
            ...
            'type' => 'required|in:dropdown,multiple_select,field',
            'attribute_group' => 'required|exists:attribute_groups,id',
            'category_ids' => ['required', new CheckChildCategory()],
            'status' => 'required|in:Active,Inactive',
            'is_filterable' => 'required|in:1,0',
            'is_required' => 'required|in:1,0',
        ];
        if (isset($data['type']) && $data['type'] != 'field') {
            $rules['values'] = ['required', new CheckAttributeValue($data['type'] ?? null)];
        }
