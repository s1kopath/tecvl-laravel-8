        ...
        if (isset($value)) {
            foreach ($value as $key => $optionValue) {
                if (count(array_diff($optionField, array_keys($optionValue))) == 0) {
                    if (strlen($optionValue['option_name']) > 0 && in_array($optionValue['type'], selecItemForOption()) && in_array($optionValue['is_required'], $requiredValue['is_required'])) {
                            ...
                            if (count($optionNameColor) <= 1) {
                                if (count($optionNameSize) <= 1) {
                                    ...
                                    if (isset($optionValue['item_option_id'])) {
                                        $itemOptionCheck = ItemOption::where('id', $optionValue['item_option_id'])->where('item_id', $this->itemId);
