        ...
        return datatables()
            ...
            ->addColumn('action', function ($templates) {
                ...
                $delete = '<form method="post" action="'. route('emailTemplates.destroy', ['id' => $templates->id]) .'" id="delete-email-template-'. $templates->id .'" accept-charset="UTF-8" class="display_inline">
                        '.csrf_field().'
                        <button title="' . __('Delete') . '" class="btn btn-xs btn-danger" type="button" data-id=' . $templates->id . ' data-label="Delete" data-delete="email-template" data-toggle="modal" data-target="#confirmDelete" data-title="' . __('Delete :x', ['x' => __('Template')]) . '" data-message="' . __('Are you sure to delete this?') . '">
                ...
                if ($this->hasPermission(['App\Http\Controllers\MailTemplateController@destroy'])) {
                    $str .= $delete;
                }
                return $str;
            })
