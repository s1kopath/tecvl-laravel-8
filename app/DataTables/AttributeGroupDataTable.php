        ...
        return datatables()
            ...
            ->addColumn('vendor', function ($attributeGroups) {
                ...
                $delete = '<form method="post" action="' . route('attributeGroup.destroy', ['id' => $attributeGroups->id]) .'" id="delete-user-'. $attributeGroups->id . '" accept-charset="UTF-8" class="display_inline">
                        ' . method_field('DELETE') . '
                        ' . csrf_field() . '
                        <button title="' . __('Delete') . '" class="btn btn-xs btn-danger" type="button" data-id=' . $attributeGroups->id . ' data-delete="user" data-label="Delete" data-toggle="modal" data-target="#confirmDelete" data-title="' . __('Delete :x', ['x' => __('Attribute Group')]) . '" data-message="' . __('Are you sure to delete this?') . '">
