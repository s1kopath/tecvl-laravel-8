        ...
        return datatables()
            ...
            ->addColumn('action', function ($roles) {
                ...
                $delete = '<form method="post" action="'. route('roles.destroy', ['id' => $roles->id]) .'" id="delete-role-'. $roles->id .'" accept-charset="UTF-8" class="display_inline">
                        '.csrf_field().'
                        <button title="' . __('Delete') . '" class="btn btn-xs btn-danger" type="button" data-id=' . $roles->id . ' data-label="Delete" data-delete="role" data-toggle="modal" data-target="#confirmDelete" data-title="' . __('Delete :x', ['x' => __('Role')]) . '" data-message="' . __('Are you sure to delete this?') . '">
