...
class UserListDataTable extends DataTable
{
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('picture', function ($users) {
                return '<img src="' . $users->fileUrl() . '" alt="" width="50" height="50">';
            })
            ...
            ->addColumn('action', function ($users) {
                ...
                $delete = '<form method="post" action="' . route('users.destroy', ['id' => $users->id]) . '" id="delete-user-' . $users->id . '" accept-charset="UTF-8" class="display_inline">
                        ' . csrf_field() . '
                        <button title="' . __('Delete') . '" class="btn btn-xs btn-danger" type="button" data-id=' . $users->id . ' data-delete="user" data-label="Delete" data-toggle="modal" data-target="#confirmDelete" data-title="' . __('Delete :x', ['x' => __('User')]) . '" data-message="' . __('Are you sure to delete this?') . '">
                ...
                if ($this->hasPermission(['App\Http\Controllers\UserController@destroy']) && optional(optional($users->role)->roles)->slug <> 'super-admin') {
                    $str .= $delete;
                }
                return $str;
            })
    ...
    {
        $users = User::query()->with(['role'])->filter();
