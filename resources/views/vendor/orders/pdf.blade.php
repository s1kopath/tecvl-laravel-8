    ...
    <table class="list-table">
        ...
        @foreach($orders as $key => $order)
            <tr>
                ...
                <td class="text-center list-td"> {{ formatCurrencyAmount($order->getTotalVendorItem(session()->get('vendorId'), $order->id)) }} </td>
                <td class="text-center list-td"> {{ formatCurrencyAmount($order->vendorItemPrice(session()->get('vendorId'), $order->id)) }} </td>
