    ...
    <section class="mx-4 lg:mx-4 xl:mx-32 2xl:mx-64 3xl:mx-92">
        <nav class="text-gray-600 text-sm mt-5 md:mt-30p" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex text-xs md:text-sm roboto-medium font-medium text-gray-10">
                <li class="flex items-center">
                    ...
                    <a href="http://localhost/multivendor">Home</a>
                    <p class="px-7p">/</p>
                </li>
                <li class="flex items-center">
                    <a class="text-gray-12" href="http://localhost/multivendor/carts">Cart</a>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex justify-center items-center mt-5 -ml-5 md:-ml-0">
            <div class="uppercase relative">
                ...
                <p class="absolute -left-1.5 md:-left-5 mt-2.5 md:mt-5 whitespace-nowrap dm-bold font-bold text-gray-12 text-8 md:text-base">My Cart</p>
            </div>
            <div class="uppercase relative">
                <div class="flex justify-center items-center">
                ...
                <p class="absolute -left-2.5 md:-left-4 mt-2.5 md:mt-5 whitespace-nowrap dm-bold font-bold text-gray-10 text-8 md:text-base">Checkout</p>
            </div>
            <div class="uppercase relative">
                <div class="flex justify-center items-center">
                    <div class="md:h-9 md:w-9 h-5 w-5 bg-gray-11 rounded-full flex justify-center items-center">
                ...
                <p class="absolute -left-2.5 md:-left-4 mt-2.5 md:mt-5 whitespace-nowrap dm-bold font-bold text-gray-10 text-8 md:text-base">Payment</p>
            </div>
            <div class="uppercase relative">
                <div class="flex justify-center items-center">
                    <div class="md:h-9 md:w-9 h-5 w-5 bg-gray-11 rounded-full flex justify-center items-center">
                ...
                <p class="absolute -left-7 md:-left-14 mt-2.5 md:mt-5 whitespace-nowrap dm-bold font-bold text-gray-10 text-8 md:text-base">Order complete</p>
            </div>
        </div>
    </section>
    ...
    <section class="mx-5 lg:mx-4 xl:mx-32 2xl:mx-64 3xl:mx-92 mt-12 break-all">
                <div class="rounded-md grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5  mb-7 lg:mb-8 xl:mb-10 gap-5">
                        <div class="md:border-r mt-3md:border-gray-2  last:border-0 dm-sans font-medium text-18 text-gray-12 pr-2">
                            <span class="uppercase roboto-medium font-medium text-13 text-gray-10 block mb-1.5 md:mb-0 whitespace-nowrap">{{ __('Invoice Number') }} </span>{{ $order->reference }}
                        </div >
                        ...
                        <span class="uppercase roboto-medium font-medium text-13 text-gray-10 block mb-1.5 md:mb-0 whitespace-nowrap">{{ __('Payment Method') }} </span>{{ optional($order->paymentMethod)->gateway }}
                        </div>
                </div>
    </section>
                    ...
                    <div class="border border-gray-2 px-4 md:px-7 rounded">
                        <table class="w-full font-semibold text-sm lg:text-base">
                            ...
                            <tr class="font-normal last:border-b-0">
                                <td colspan="2" class="dm-sans font-medium text-gray-12 text-sm  {{ $loop->first ? 'pt-6' : ''}} {{ $loop->last ? 'pb-6' : 'pb-3' }}">
                                    <div class="flex justify-between flex-col md:flex-row">
                                        <div>
                                            {{ $detail->item_name }} <span class="mx-2 text-sm">x</span> {{ formatCurrencyAmount($detail->quantity) }}
                                            @if(isset($detail->vendor->name))
                                              <span class="block text-xs roboto-medium font-medium text-gray-10 pt-1">{{ __('Vendor') }}: {{ $detail->vendor->name }}</span>
                                            @endif
                                        </div>
                            ...
                            <tr class="odd:bg-skin-secondary border-t border-gray-2 dm-sans font-medium text-gray-12">
                                ...
                                <td class="text-right">{{ formatNumber($order->other_discount_amount, optional($order->currency)->symbol) }} </td>
                            </tr>
                            @endif
