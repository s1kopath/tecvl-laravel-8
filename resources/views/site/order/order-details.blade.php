...
@section('content')
    <div class="dark:bg-red-1 h-full xl:px-74p px-5 pt-30p xl:pt-14">
        ...
        <div class="lg:mt-70p mt-8 xl:flex justify-between">
            <div class="mr-3">
                <div class="flex flex-row lg:flex-col justify-between">
                    <div class="">
                        ...
                        <h3 class="uppercase dm-bold font-bold text-gray-12 lg:text-28 text-xl">{{ $order->reference }}</h3>
                    </div>
                    <div class="lg:mt-10">
                        <p class="uppercase roboto-medium font-medium lg:text-lg text-15 lg:text-left text-right text-gray-10">{{ __('Status') }}</p>
                        <h3 class="uppercase dm-bold font-bold text-gray-12 lg:text-28 text-xl">{{ $order->orderStatus->name }}</h3>
                    </div>
                </div>
                <div class="flex flex-row lg:flex-col justify-between">
                    <div class="mt-10">
                        <p class="uppercase roboto-medium font-medium lg:text-lg text-15 text-gray-10">{{ __('Shipping method') }}</p>
                        <h3 class="uppercase dm-bold font-bold text-gray-12 lg:text-28 text-xl">
                             {{ implode(', ', $shippingMethods) }}</h3>
                    </div>
                    <div class="mt-10 text-left">
                        <p class="uppercase roboto-medium font-medium lg:text-lg text-15 lg:text-left text-right text-gray-10">{{ __('Door Drop') }}</p>
                        <h3 class="uppercase dm-bold font-bold text-gray-12 lg:text-28 text-xl"> {{ !empty($order->leave_door) ? $order->leave_door : __('Unknown') }}</h3>
                    </div>
                </div>
            </div>
            <div class="grid 2xl:grid-cols-3 lg:grid-cols-2 grid-cols-1 lg:gap-50p gap-5 mt-7 lg:mt-0">
                <div class="lg:pl-30p pl-5 border border-gray-2 lg:pr-74p pr-0 lg:pb-36 pb-6">
                    ...
                    <p class="roboto-medium font-medium text-gray-10 mt-11 lg:mb-38p mb-7">{{ __('status') }}:
                        <span class="text-green-1 bg-green-2 px-6 py-2 rounded-sm dm-sans font-medium text-xl ml-4">{{$order->payment_status }}</span>
                    </p>
                    <div>
                        <P class="roboto-medium font-medium text-gray-10 mb-1.5">{{ __('Paid Amount') }}: {{ formatNumber($order->paid) }}</P>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ __('Amount to be Paid') }}: {{ formatNumber($order->total) }}</p>
                        <p class="uppercase roboto-medium font-medium text-gray-10">{{ __('VAT') }}:
                            <span>{{ formatNumber($order->tax_charge) }}</span>
                        </p>
                    </div>
                    <p class="dm-sans font-medium text-gray-12 lg:mt-8 mt-5">{{ __('Payment Method') }}: {{ !empty($order->paymentMethod->gateway) ? $order->paymentMethod->gateway : __('Unknown') }}</p>
                ...
                <div class="lg:pl-30p pl-5 border border-gray-2 lg:pr-74p pr-0 lg:pb-36 pb-6">
                    ...
                    <div class="mt-8">
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ optional($order->address)->first_name . ' ' . optional($order->address)->last_name }}</p>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5"> {{ optional($order->address)->phone }}
                        </p>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5"> {{ optional($order->address)->address_1 }}
                            {{ !empty(optional($order->address)->address_2) ? ', ' . optional($order->address)->address_2 : '' }},
                            {{ optional($order->address)->city }}</p>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ __('Postcode') }}: {{ optional($order->address)->zip }}</p>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ __('Country') }}:{{ optional($order->address)->country }}</p>
                    </div>
                    <p class="mt-30p dm-sans font-medium text-gray-12">{{ __('Email') }}: {{ optional($order->address)->email }}</p>
                    <div>
                        <p class="mt-30p dm-sans font-medium text-gray-12">{{ __('Delivery Instructions') }}:</p>
                        <p class="mt-2 roboto-medium font-medium text-gray-10 text-sm pr-5">{{ $order->note }}</p>
                    </div>
                </div>
                <div class="lg:pl-30p pl-5 border border-gray-2 pb-6 lg:pb-36">
                    <div class="flex mt-11">
                        <span class="mr-4">
                    ...
                    <div class="mt-8">
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ optional($order->user)->name }}</p>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ optional($order->user)->phone }}</p>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ optional($order->user)->address }}
                        </p>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ __('Postcode') }}: {{ optional($order->address)->zip }}</p>
                        <p class="roboto-medium font-medium text-gray-10 mb-1.5">{{ __('Country') }}: {{ optional($order->address)->country }}</p>
                    </div>
                    <p class="mt-30p dm-sans font-medium text-gray-12"></p>{{ __('Email') }}: {{ optional($order->user)->email }} </p>
                </div>
            </div>
        </div>
        <div class="overflow-x-auto hidden xl:block rounded-sm mt-16">
            <table class="w-full bg-white dark:bg-gray-2 overflow-hidden table-striped dark:border-gray-0">
                <thead>
                    ...
                    <tr class="text-left bg-gray-11 border border-gray-2 dark:bg-gray-2">
                        <th class="px-6 py-4 text-gray-12 dm-sans font-medium text-xl dark:text-gray-2">
                            {{ __('Items') }}
                        </th>
                        <th class="px-6 py-4 text-gray-12 dm-sans font-medium text-xl dark:text-gray-2">
                            {{ __('Status') }}</th>
                    ...
                    @foreach ($detailGroups as $group)
                        <tr class="focus-within:bg-gray-200 border border-gray-2 overflow-hidden">
                            <td class="dark:border-t-gray-2 dark:bg-gray-3">
                                <div>
                                    @foreach ($group as $detail)
                                        @if ($loop->iteration == 1)
                                            <div class="flex mt-7">
                                                <p class="roboto-medium font-medium text-gray-10 pl-6 pr-3.5 flex items-center dark:text-gray-2">{{ __('Seller') }}: {{ $detail->shop->name }}</p>
                                                <a class="flex rounded-sm text-sm roboto-medium h-7 font-medium bg-yellow-1 py-1 px-2.5" href="#">
                                                    <svg class="mt-1 mr-1.5" xmlns="http://www.w3.org/2000/svg" width="13"
                                                        height="12" viewBox="0 0 13 12" fill="none">
                                                        <path d="M8.75217 0.0375013C8.64553 0.0562515 8.46779 0.100782 8.35861 0.140627C7.91681 0.297658 8.06154 0.173439 4.84201 3.14063C3.11291 4.73203 1.81291 5.94844 1.75705 6.02344C1.70627 6.09375 1.63771 6.21094 1.60724 6.28594C1.57424 6.35859 1.43205 6.92344 1.2924 7.53984C0.995329 8.83125 0.980095 8.96719 1.09689 9.27188C1.1756 9.47578 1.31271 9.67266 1.4676 9.79922C1.68088 9.97266 2.04904 10.1016 2.3258 10.1016C2.42228 10.1016 4.59572 9.71484 4.96896 9.63047C5.03752 9.61406 5.17717 9.56484 5.28127 9.52031C5.46916 9.43828 5.52502 9.38906 8.47287 6.67031C11.6365 3.75 11.6162 3.77109 11.7762 3.41484C11.8904 3.15469 11.9362 2.95547 11.9514 2.64844C11.9717 2.16797 11.8498 1.74844 11.5705 1.36641C11.3903 1.11563 10.6235 0.410158 10.4102 0.297658C9.9176 0.0375013 9.31838 -0.0562487 8.75217 0.0375013ZM9.359 1.2211C9.62053 1.26563 9.76271 1.35469 10.1613 1.72969C10.56 2.10469 10.6387 2.21719 10.6895 2.47734C10.7225 2.65781 10.6742 2.90625 10.5701 3.06797C10.5295 3.13359 9.37424 4.22344 7.61467 5.85938L4.72521 8.54297L3.52931 8.72813C2.87424 8.83125 2.33088 8.90859 2.3258 8.90391C2.32072 8.89922 2.42228 8.4 2.55177 7.79766L2.78537 6.69844L5.66467 4.04063C8.18596 1.71094 8.56174 1.37344 8.69631 1.3125C8.93244 1.20234 9.11017 1.17891 9.359 1.2211Z"
                                                            fill="#2C2C2C"/>
                                                        <path
                                                            d="M1.42692 10.8816C1.03844 11.0129 0.916569 11.5051 1.20094 11.8051C1.24157 11.8496 1.32536 11.9082 1.38122 11.9363L1.48532 11.9879L6.39841 11.9949C11.8675 11.9996 11.4791 12.0113 11.6847 11.8402C11.7431 11.7934 11.8092 11.7113 11.8345 11.6598C11.8904 11.5473 11.898 11.327 11.8472 11.2168C11.7939 11.0973 11.6543 10.966 11.5146 10.9074L11.3877 10.8512L6.44919 10.8535C2.71423 10.8535 1.4904 10.8606 1.42692 10.8816Z"
                                                            fill="#2C2C2C"/>
                                                    </svg>{{ __('Manage') }}
                                                </a>
                                            </div>
                                    ...
                                    <div>
                                        @foreach ($group as $detail)
                                            ...
                                            @endphp
                                        <div class="justify-center items-center flex mb-8 mt-5">
                                            <span class="bg-gray-17 ml-6 h-16 w-16 rounded-sm flex justify-center items-center">
                                                <img src="{{ $detail->item->fileUrl() }}" alt="item"
                                                    class="p-4"/>
                                            </span>
                                            <div class="ml-8">
                                                <a
                                                    href="{{ route('site.itemDetails', ['code' => optional($detail->item)->item_code, 'name' => cleanedUrl($detail->item_name)]) }}">
                                                    <span
                                                        class="dm-sans font-medium text-gray-12 pt-2 flex items-center dark:text-gray-2">
                                                        {{ trimWords($detail->item_name, 25) }} </span>
                                                    <span
                                                        class="text-sm mt-2 roboto-medium font-medium text-gray-10">{{ !empty($opName) ? $opName : '' }}</span>
                                                </a>
                                            </div>
                                        </div>
                                        @endforeach
                            ...
                            <td class="dark:border-t-gray-2 dark:bg-gray-3">
                                @foreach ($group as $detail)
                                <div class="mt-10">
                                    <span class="roboto-medium font-medium text-gray-12 px-6 py-4 flex items-center dark:text-gray-2">
                            ...
                            <td class="dark:border-t-gray-2 dark:bg-gray-3">
                                @foreach ($group as $detail)
                                    <div class="mt-10">
                                        <span class="roboto-medium font-medium text-gray-12 px-6 py-4 flex items-center dark:text-gray-2">
                                            {{ formatNumber($detail->quantity * $detail->price) }}
                                        </span>
                                    </div>
                                @endforeach
                            </td>
                            ...
                            <td class="dark:border-t-gray-2 dark:bg-gray-3">
                                ...
                                @else
                                <div class="mt-12">
                                    <a class="text-gray-10 ml-6 mt-5 w-90p text-sm roboto-medium justify-center items-center font-medium mr-11 flex bg-gray-300 py-1.5 rounded-sm cursor-not-allowed"
                                        href="#">
                                        <svg class="mr-2.5" xmlns="http://www.w3.org/2000/svg" width="14"
                                            height="16" viewBox="0 0 14 16" fill="none">
                                            <path
                ...
                <tfoot>
                    <tr class="focus-within:bg-gray-200 border border-gray-2 overflow-hidden">
                        ...
                        <td class="dark:border-t-gray-2 dark:bg-gray-3 dm-sans font-medium text-gray-12 px-6 py-18p"></td>
                        @if (isActive('Shop'))
                            <td class="dark:border-t-gray-2 dark:bg-gray-3 dm-sans font-medium text-gray-12 px-6 py-18p">
                            </td>
                        @endif
                        <td class="dark:border-t-gray-2 dark:bg-gray-3 dm-sans font-medium text-gray-12 px-6 py-18p">
                            {{ __('Subtotal') }}:</td>
                        <td class="dark:border-t-gray-2 dark:bg-gray-3 dm-sans font-medium text-gray-12 px-6 py-18p">
                            {{ formatNumber($order->total + $order->other_discount_amount - ($order->shipping_charge + $order->tax_charge)) }}
                        </td>
                    </tr>
                    <tr class="dark:border-t-gray-2 dark:bg-gray-3 border border-gray-2"></tr>
                    <tr
                        class="focus-within:bg-gray-200 overflow-hidden dark:border-t-gray-2 dark:bg-gray-3 border border-gray-2">
                        <td></td>
                        <td></td>
                        <td></td>
                        @if (isActive('Shop'))
                            <td></td>
                        @endif
                        <td class="dm-sans font-medium text-gray-12 px-6 py-18p">{{ __('Shipping') }}:</td>
                    ...
                    @if ($order->other_discount_amount > 0 && isActive('Coupon'))
                        ...
                        <tr class="focus-within:bg-gray-200 overflow-hidden border border-gray-2">
                            ...
                            @if (isActive('Shop'))
                                <td></td>
                            @endif
                            <td>{{ __('Coupon offer') }}:</td>
                            <td>{{ formatNumber($order->other_discount_amount) }}
                            </td>
                        </tr>
                    @endif
                    <tr class="dark:border-t-gray-2 dark:bg-gray-3 border border-gray-2"></tr>
        ...
        <div class="block xl:hidden mt-30p">
            ...
            <div class="mt-3 mb-5 border border-gray-2 rounded-md">
                ...
                <div class="flex justify-between mx-4 mt-4">
                    <div>
                        <p class="roboto-medium font-medium text-gray-12 text-13 leading-none flex items-center dark:text-gray-2">
                            {{ __('Seller') }}: {{ $detail->shop->name }}
                        </p>
                        <span
                            class="leading-none pt-1 roboto-medium font-medium text-gray-10 text-xs dark:text-gray-2">{{ __('Status') }}:
                            {{ $detail->orderStatus->name }}
                        </span>
                    </div>
                    <a class="flex rounded-sm text-11 h-26p roboto-medium font-medium bg-yellow-1 justify-center items-center px-3"
                        href="#">
                        <svg class=" mr-1" xmlns="http://www.w3.org/2000/svg" width="11" height="10"
                            viewBox="0 0 13 12" fill="none">
                            <path
                                d="M8.75217 0.0375013C8.64553 0.0562515 8.46779 0.100782 8.35861 0.140627C7.91681 0.297658 8.06154 0.173439 4.84201 3.14063C3.11291 4.73203 1.81291 5.94844 1.75705 6.02344C1.70627 6.09375 1.63771 6.21094 1.60724 6.28594C1.57424 6.35859 1.43205 6.92344 1.2924 7.53984C0.995329 8.83125 0.980095 8.96719 1.09689 9.27188C1.1756 9.47578 1.31271 9.67266 1.4676 9.79922C1.68088 9.97266 2.04904 10.1016 2.3258 10.1016C2.42228 10.1016 4.59572 9.71484 4.96896 9.63047C5.03752 9.61406 5.17717 9.56484 5.28127 9.52031C5.46916 9.43828 5.52502 9.38906 8.47287 6.67031C11.6365 3.75 11.6162 3.77109 11.7762 3.41484C11.8904 3.15469 11.9362 2.95547 11.9514 2.64844C11.9717 2.16797 11.8498 1.74844 11.5705 1.36641C11.3903 1.11563 10.6235 0.410158 10.4102 0.297658C9.9176 0.0375013 9.31838 -0.0562487 8.75217 0.0375013ZM9.359 1.2211C9.62053 1.26563 9.76271 1.35469 10.1613 1.72969C10.56 2.10469 10.6387 2.21719 10.6895 2.47734C10.7225 2.65781 10.6742 2.90625 10.5701 3.06797C10.5295 3.13359 9.37424 4.22344 7.61467 5.85938L4.72521 8.54297L3.52931 8.72813C2.87424 8.83125 2.33088 8.90859 2.3258 8.90391C2.32072 8.89922 2.42228 8.4 2.55177 7.79766L2.78537 6.69844L5.66467 4.04063C8.18596 1.71094 8.56174 1.37344 8.69631 1.3125C8.93244 1.20234 9.11017 1.17891 9.359 1.2211Z"
                                fill="#2C2C2C"/>
                            <path
                                d="M1.42692 10.8816C1.03844 11.0129 0.916569 11.5051 1.20094 11.8051C1.24157 11.8496 1.32536 11.9082 1.38122 11.9363L1.48532 11.9879L6.39841 11.9949C11.8675 11.9996 11.4791 12.0113 11.6847 11.8402C11.7431 11.7934 11.8092 11.7113 11.8345 11.6598C11.8904 11.5473 11.898 11.327 11.8472 11.2168C11.7939 11.0973 11.6543 10.966 11.5146 10.9074L11.3877 10.8512L6.44919 10.8535C2.71423 10.8535 1.4904 10.8606 1.42692 10.8816Z"
                                fill="#2C2C2C"/>
                ...
                <div>
                    <div class="flex justify-between items-center mx-4 mb-4">
                        <div class="flex justify-center items-center mt-4">
                            <span class=" flex items-center mr-3 bg-gray-17 rounded">
                                <img src="{{ $detail->item->fileUrl() }}" alt="item"
                                    class="flex w-8 h-8 m-3 justify-center text-center"/>
                            </span>
                            <div>
                                <p class="roboto-medium text-xs leading-none font-medium text-gray-12 flex items-center dark:text-gray-2">
                                    {{ trimWords($detail->item_name, 25) }}
                                </p>
                                <p class="text-xss mt-5p roboto-medium leading-none font-medium text-gray-10">
                                    {{ !empty($opName) ? $opName : '' }}</p>
                                <p class="dm-sans mt-5p font-medium text-xss text-gray-12">
                                    <span
                                        class="mr-1">{{ formatNumber($detail->price) }}</span>
                                    <span class="mr-1">x {{ formatCurrencyAmount($detail->quantity) }} </span>
                                    <span> (
                                        {{ formatNumber($detail->quantity * $detail->price) }})</span>
                                </p>
                            </div>
                        </div>
                        @if ($detail->isRefundable())
                        <div>
                            <a href="#" class="text-gray-10 justify-center items-center flex border border-gray-2 p-1.5 rounded-sm">
                                <svg
                                    class="" xmlns="http://www.w3.org/2000/svg" width="13" height="14"
                        ...
                        @else
                        <div>
                            <a href="#" class="text-gray-10 justify-center items-center flex border border-gray-300 p-1.5 rounded-sm cursor-not-allowed">
                                <svg
                                    class="" xmlns="http://www.w3.org/2000/svg" width="13" height="14"
                                    viewBox="0 0 14 16" fill="none">
                                    <path
