...
@section('content')
    <div class="dark:bg-red-1 h-full xl:px-74p px-5 pt-30p xl:pt-14">
        ...
        @if ($addresses->count() > 0)
            <div class="2xl:mr-89p  mt-5">
                <div class="flex  justify-between">
                    <div class="dm-sans lg:mt-14 mr-15p justify-center items-center  flex dm-bold font-bold text-gray-12 lg:text-2xl text-lg uppercase">
                        <p>{{ __('Saved addresses') }}</p>
                    </div>
                    @if (count($addresses) > 0)
                        <div class="flex lg:mt-10 my-3 mb-3 rounded justify-center items-center border border-gray-12  ">
                ...
                <div class="grid 2xl:grid-cols-3 lg:grid-cols-2 grid-cols-1 3xl:gap-x-50p 2xl:gap-x-6 lg:gap-x-5 lg:gap-y-10 gap-y-5 ">
                        ...
                        <div class="w-full h-full mb-6 lg:pl-30p px-5 border {{ $address->is_default == 1 ? 'border-yellow-1 bg-yellow-4 dark:border-gray-0 dark:bg-gray-3' : 'border' }}  rounded">
                                ...
                                <div x-data="{ showModal1: false }" :class="{ 'overflow-y-hidden': showModal1 }">
                                    ...
                                    <div class="fixed inset-0 w-full h-full bg-black bg-opacity-50 pt-60 duration-300 overflow-y-auto" x-show="showModal1" x-transition:enter="transition duration-300"
                                        ...
                                        <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/2 xl:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
                                            <div class="relative  bg-white shadow-lg rounded-md text-gray-900 z-50 " @click.away="showModal1 = false" x-show="showModal1" x-transition:enter="transition transform duration-300"x-transition:enter-start="scale-0" x-transition:enter-end="scale-100"  x-transition:leave="transition transform duration-300" x-transition:leave-start="scale-100" x-transition:leave-end="scale-0">
                                                <div class=" grid lg:grid-cols-4 grid-cols-2  gap-32 items-center">
                                                    <div class="flex col-span-3">
                                                        <div class="flex flex-col justify-center bg-red-100 mt-30p ml-30p items-center h-10 w-10 rounded-full dark:text-gray-2 ">
                                                            ...
                                                            <svg class="lg:hidden block" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none">
                                                        ...
                                                        <span class="inline-block leading-4 ml-2 mt-10 dm-sans font-medium lg:text-lg text-gray-12 lg:mb-0 mb-0.5 lg:pr-0 pr-3 text-sm  ">{{ __('Are you sure you want to delete this?') }}</span>
                                                    </div>
                                                    <svg class="lg:block hidden mr-30p mt-7 right-0 cursor-pointer hover:text-gray-12 text-gray-10"
                                                ...
                                                <p class=" text-gray-10 -mt-1 roboto-medium font-medium lg:text-sm lg:ml-77p text-11  ml-20  lg:pr-0 pr-10">
                                                    {{ __('Please keep in mind that once deleted, you can not undo it.') }}
                                                </p>
                                                <div class=" flex justify-end lg:mt-8 lg:mr-30p lg:mx-0 mx-2 lg:pb-7 pb-5 mt-6">
                                                    <button class="dm-sans items-center transition duration-200 rounded pb-4 pt-3  cursor-pointer font-medium lg:text-sm text-gray-12 lg:w-141p lg:h-46p w-36 h-10 bg-white border border-gray-2 mb-7p text-xs hover:border-gray-12"  @click="showModal1 = false"> {{ __('Cancel') }}
                                                    </button>
                                                    <form action="{{ route('site.addressDelete', ['id' => $address->id]) }}"  method="post">
                                                        @csrf
                                                        <button type="submit" class="dm-sans ml-3 transition duration-200 items-center cursor-pointer py-3.5 lg:px-6 px-0 font-medium lg:text-sm text-white lg:w-141p lg:h-46p bg-gray-12 hover:bg-yellow-1 hover:text-gray-12  mb-7p    text-xs  w-36  h-10  rounded">
                                                            {{ __('Yes, Delete') }}
                                                        </button>
                                                    </form>
                                                </div>
                            ...
                            <div class="mt-8">
                                ...
                                <div class=" roboto-medium font-medium  text-base capitalize break-all mb-5 text-gray-10">
                                    ...
                                    <p>{{ __('country') }}: {{ $address->country }}</p>
                                </div>
                                <div class="dm-sans font-medium text-gray-12   text-base">
                                    <p class="break-all capitalize mb-2.5">phone: {{ $address->phone }}</p>
                            ...
                            <div class="flex mt-49p">
                                @if ($address->is_default == 1)
                                    ...
                                    <p class="dm-sans font-medium text-base -mt-1 ml-2 text-gray-12">
                                        {{ __('Used as default address') }}
                                    </p>
                                @else
        ...
        <div class="flex">
            <div class="flex lg:mt-75p mt-10 dm-bold font-bold text-gray-12 lg:text-2xl text-base uppercase">
                <svg class="mr-3 mt-1 lg:block hidden " xmlns="http://www.w3.org/2000/svg" width="20" height="21"
        ...
        <div class="items-center xl:w-1/2 lg:w-ful lg:mt-27p mt-5">
            <div>
                <form action="{{ route('site.addressStore') }}" method="post">
                    ...
                    <div class=" lg:mb-3 mb-3.5  ">
                        <label class=" text-sm dm-sans font-medium capitalize text-gray-12 " for="company">
                            {{ __('Company Name (Optional)') }} </label>
                    ...
                    <div class=" lg:mb-3 mb-3.5">
                        <label class="lg:block hidden mt-3p text-sm dm-sans font-medium capitalize text-gray-12 require-profile "  for="address_1"> {{ __('Street Address 1') }} </label>
                    ...
                    <div class=" lg:block hidden lg:mb-3 mb-3.5 ">
                        <label class=" text-sm dm-sans font-medium capitalize text-gray-12 " for="address_2">
                            {{ __('Street Address 2') }}</label>
                    ...
                    <div class="grid grid-cols-2 lg:gap-3 gap-15p">
                        <div class="lg:mb-3 mb-3.5 ">
                            <label class=" text-sm dm-sans font-medium capitalize text-gray-12 require-profile" for="city">
                                {{ __('City ') }}</label>
                            <input id="city" name="city" type="text" value="{{ old('city') }}"  class="border-gray-2 rounded-sm w-full lg:h-46p mt-1.5 lg:mt-1p h-10 roboto-medium pl-18p font-medium text-sm text-gray-10 form-control border focus:border-gray-12" required oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')">
                        </div>
                        <div class="lg:mb-3 mb-3.5 ">
                            ...
                            <input id="zip" name="zip" type="text" value="{{ old('zip') }}" class="border-gray-2 rounded-sm w-full lg:h-46p mt-2 lg:mt-1p h-10 roboto-medium pl-18p font-medium text-sm text-gray-10 form-control border focus:border-gray-12">
                        </div>
                    </div>
                    <div class="grid grid-cols-2 lg:gap-3  gap-15p   ">
                        <div class="w-full">
                            <label class=" text-sm dm-sans font-medium capitalize text-gray-12 require-profile" for="country"> {{ __('Country ') }} </label>
                            <input class="border-gray-2 mt-1.5 lg:mt-1p rounded-sm w-full lg:h-46p h-10 roboto-medium pl-18p font-medium text-sm text-gray-10 form-control border focus:border-gray-12" value="{{ old('country') }}" type="text" id="country" name="country" required  oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')">
                        </div>
                        <div class="w-full ">
                            <label class=" text-sm dm-sans font-medium capitalize text-gray-12" for="state">{{ __('State') . ' / ' . __('Province') }} </label>
                            <input class="border-gray-2 mt-1.5 lg:mt-1p rounded-sm w-full lg:h-46p h-10 roboto-medium pl-18p font-medium text-sm text-gray-10 form-control border focus:border-gray-12" id="state" name="state" value="{{ old('state') }}" type="text">
                        </div>
                    </div>
                    <div class="lg:mt-15p mt-18p">
                        ...
                        <p class="lg:hidden block text-sm dm-sans font-medium capitalize text-gray-12">
                             {{ __('type of place') }} *</p>
                        ...
                        <br>
                        <p id="radio-error-msg" class="hidden font-normal text-11 text-reds-5 -mt-5 radio-error-msg">This field is required.</p>
                    </div>
                    @if ($addresses->count() > 0)
                        <div class="form-check mt-0">
                            ...
                            <label class="form-check-label inline-block  text-gray-10 lg:text-sm text-xs dm-sans font-medium " for="flexCheckDefault">
                                {{ __('Use this as default address in the future') }}
                            </label>
                        </div>
                    @endif
                    <div class="flex lg:mt-5 mt-6">
                        ...
                        <button type="submit"  class="dm-sans lg:ml-3 ml-0 transition duration-200 items-center cursor-pointer py-3.5 px-6 font-medium text-sm whitespace-nowrap  text-white w-141p h-46p bg-gray-12 save-add-func hover:bg-yellow-1 hover:text-gray-12 mb-7p rounded">{{ __('Save Address') }}</button>
                    </div>
                </form>
            </div>
        </div>
