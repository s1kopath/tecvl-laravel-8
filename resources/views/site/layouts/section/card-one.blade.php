    ...
    <div>
        <div class="border rev-img rounded-md relative">
            <div class="p-10 flex justify-center" >
                <img class="h-20 w-20 object-contain" src="{{ $item->fileUrl(['home' => true]) }}" alt="">
            </div>
            ...
            <div class="opacity-0 hover:opacity-100 duration-300 absolute inset-0 z-10 flex justify-center items-center text-xl text-white font-semibold">
                <div class="absolute flex justify-center h-6 w-6 cursor-pointer top-2 right-6">
                    <a href="javascript:void(0)" class="relative sm:px-4 py-2 block w-fill">
                        <div slot="icon" class="relative">
                            {{-- Add to cart --}}
                            @if(!hasOption($item->id))
                                <a href="javascript:void(0)" class="add-to-cart" data-itemId={{ $item->id }}>
                                    <div class="h-6 w-6 p-1 text-gray-12 hover:bg-yellow-1 border border-gray-2 rounded-full">
                            ...
                            @endphp
                            {{-- Wishlist --}}
                            <div data-id="{{ $item->id }}" class="wishlist h-6 w-6 p-1 mt-2 text-gray-12 hover:bg-yellow-1 border border-gray-2 rounded-full bg-white {{ $wishlisted ? 'remove-wishlist bg-yellow-1' : 'add-wishlist' }}">
                            ...
                            {{-- Compare --}}
                            <div data-itemId="{{ $item->id }}" class="h-6 w-6 p-1 mt-2 text-gray-12 hover:bg-yellow-1 border border-gray-2 rounded-full bg-white add-to-compare">
                                <svg class="mt-1" width="13" height="9" viewBox="0 0 13 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M8.36875 3.28795V5.12946L5.875 2.67411L8.36875 0.21875V2.06027H12.75V3.28795H8.36875ZM0.25 5.7433H4.63125V3.90179L7.125 6.35714L4.63125 8.8125V6.97098H0.25V5.7433Z" fill="#2C2C2C"/>
                                </svg>
                            </div>
                        </div>
                    </a>
                </div>
                <button class="open-view-modal" data-itemId="{{ $item->id }}">
                    <p class="text-gray-12 font-medium absolute inset-x-0 bottom-0 p-1 text-center text-sm bg-yellow-1">{{ __('Quick View') }}</p>
                </button>
            </div>
        </div>
        <div class="sm:text-center flex flex-col">
            <a href="{{ route('site.itemDetails', ['code' => $item->item_code, 'name' => cleanedUrl($item->name)]) }}">
                <p class="text-13 md:text-sm text-gray-12 md:mt-2 dm-regular">{{ trimWords($item->name, 30) }}</p>
            </a>
                ...
                <div class="self-top">
                    <ul class="flex sm:justify-center -space-x-1">
                        @for($i = 1; $i <= 5; $i++)
                            ...
                            {{-- Full star --}}
                                <li class="mt-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-yellow-1" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            {{-- Half star --}}
                                <li class="mt-1 pr-2">
                                    <svg class="h-3 w-3" viewBox="0 0 142 142" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M71 0L86.9405 49.0598H138.525L96.7923 79.3804L112.733 128.44L71 98.1196L29.2672 128.44L45.2077 79.3804L3.47499 49.0598H55.0595L71 0Z" fill="#C4C4C4"/>
                                        <mask id="mask0_2170_1814" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="3" y="0" width="136" height="129">
                                            <path d="M71 0L86.9405 49.0598H138.525L96.7923 79.3804L112.733 128.44L71 98.1196L29.2672 128.44L45.2077 79.3804L3.47499 49.0598H55.0595L71 0Z" fill="#C4C4C4"/>
                                        </mask>
            ...
            @if ($item->isDiscountable())
                <p class="text-13 md:text-sm text-gray-12 dm-sans">
                    {{  formatNumber($item->discounted_price) }}
                </p>
            @endif
            <p class="mt-1 md:mt-0 {{ ($item->isDiscountable()) ? 'text-11 font-medium line-through text-gray-10 pl-1 mt-0.5' : 'text-13 md:text-sm text-gray-12 dm-sans' }}">
