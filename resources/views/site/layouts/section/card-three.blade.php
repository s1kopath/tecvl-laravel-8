    ...
    <div class="border-gray-600">
        <div class="relative group">
            @if (array_key_exists($item->id, $itemFiles) && file_exists('public/uploads/items/'. $itemFiles[$item->id]))
                <img src="{{ asset('public/uploads/items/'. $itemFiles[$item->id]) }}" alt="" class="w-full h-44 sm:h-60" />
            @else
                <img src="{{ asset('public/dist/img/default-image.png') }}" alt="" class="w-full h-44 sm:h-60" />
            @endif
            <div class="layout1">
                @if (\App\Models\Item::discountPercent($item->id) > 0)
                    <div class="absolute left-2 top-2 item-percent">
                        <h4 class="px-3 py-1 rounded text-base font-medium text-green-500 bg-gray-200 shadow">
                            {{ \App\Models\Item::discountPercent($item->id) . '% ' . __('off') }}
                        </h4>
                    </div>
                @endif
                ...
                <div class="item-wishlist absolute inset-0 rounded opacity-0 group-hover:opacity-100 flex justify-end items-center bg-secondary bg-opacity-85 transition-opacity">
                    <div>
                        <div class="">
                            <div x-data="{ tooltip: false }" class="relative z-30 inline-flex wishlist bg-gray-100" data-id="{{ $item->id }}">
                                <div x-on:mouseover="tooltip = true" x-on:mouseleave="tooltip = false" class="cursor-pointer shadow px-4 py-1">
                                    <i class="fa {{ \App\Models\Item::isWishlist($item->id, optional(auth()->user())->id) ? 'fa-heart text-green-500' : 'fa-heart-o' }}"></i>
                                </div>
                                <div x-on:mouseover="tooltip = true" class="relative" x-show.transition.origin.top="tooltip" style="display: none;">
                                    ...
                                    <span>{{ \App\Models\Item::isWishlist($item->id, optional(auth()->user())->id) ? __('Remove from wishlist') : __('Add to wishlist') }}</span>
                                    </div>
                        ...
                        <div class="mt-2 item-detail">
                            <a href="{{ route('site.itemDetails', ['code' => $item->item_code, 'name' => cleanedUrl($item->name)]) }}"><button class="bg-gray-100 px-4 py-2  "><i class="fa fa-eye"></i></button></a>
                        </div>
        ...
        <div class="px-4">
            ...
            <div class="flex space-x-3 justify-center items-center">
                <div class="self-top">
                    <h3 class="text-base {{ (\App\Models\Item::isDiscountable($item->id)) ? 'line-through text-gray-500' : 'font-semibold' }} ">
                        {{ $currency_symbol }}{{ formatCurrencyAmount($item->price) }}
                    </h3>
                </div>
                @if (\App\Models\Item::isDiscountable($item->id))
                    <div>
                    ...
                    <div class="self-top">
                        <ul class="flex ">
                            ...
                            <li class="ml-1 text-gray-900">
                                {{ \App\Models\Item::rating($item->id) }} ({{ \App\Models\Item::reviewCount($item->id) }})</li>
                        </ul>
                    </div>
                </div>
