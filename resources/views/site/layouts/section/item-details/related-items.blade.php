@if ($relates->count() > 0)
    ...
    <div class="flex justify-between">
        ...
        <a class="text-gray-10 font-medium text-sm md:inline-flex items-center mt-0.5 dm-sans hidden">See More
        ...
        @foreach ($relates as $item)
            @if(!empty($item->available_from) && availableFrom($item->available_from) || empty($item->available_from))
                @if(!empty($item->available_to) && availableTo($item->available_to) || empty($item->available_to))
                    <div>
                        <div class="rev-img relative md:h-260p md:w-auto w-32 h-32">
                            <div class="border rounded-md flex justify-center absolute z-ng inset-0 md:p-14 p-7" >
                                <img class="object-contain" src="{{ $item->fileUrl() }}" alt="">
                                @if($item->isDiscountable())
                                    @if($item->discount_type == "Percent")
                                    <div class="left-3 top-3 absolute text-xs rounded-sm roboto-medium font-medium text-gray-12 text-center py-1 bg-yellow-1 px-2.5">{{ formatCurrencyAmount($item->discount_amount) . $item->discount_type }} {{ __('Off') }}</div>
                                    @else
                                        <div class="left-3 top-3 absolute text-xs rounded-sm roboto-medium font-medium text-gray-12 text-center py-1 bg-yellow-1 px-2.5">{{ $currency_symbol . formatCurrencyAmount($item->discount_amount) }} {{ __('Off') }}</div>
                                    @endif
                                @endif
                            </div>
                            ...
                            <div class="opacity-0 hover:opacity-100 duration-300 absolute inset-0 z-10 flex justify-center items-center text-xl text-white font-semibold">
                                <div class="absolute flex justify-center h-6 w-6 cursor-pointer top-2 right-4 md:top-3 md:right-6">
                                    <div slot="icon" class="relative">
                                        @if(!hasOption($item->id))
                                        <div class="h-6 w-6 md:h-8 md:w-8 p-1.5 flex flex-col justify-center items-center text-gray-12 hover:bg-yellow-1 border border-gray-2 rounded-full add-to-cart" data-itemId={{ $item->id }}>
                                        ...
                                        <div data-id="{{ $item->id }}" class="wishlist h-6 w-6 md:h-8 md:w-8 flex flex-col justify-center items-center p-1.5 mt-2 text-gray-12 hover:bg-yellow-1 border border-gray-2 rounded-full bg-white {{ $wishlisted ? 'remove-wishlist bg-yellow-1' : 'add-wishlist' }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                                            </svg>
                                        </div>

                                        <div class="h-6 w-6 md:h-8 md:w-8 flex flex-col justify-center items-center p-1.5 mt-3 text-gray-12 hover:bg-yellow-1 border border-gray-2 rounded-full bg-white add-to-compare" data-itemId="{{ $item->id }}">
                                ...
                                <a href="{{ route('site.itemDetails', ['code' => $item->item_code, 'name' => cleanedUrl($item->name)]) }}"><p class="text-gray-12 font-medium absolute inset-x-0 bottom-0 p-1 text-center text-sm bg-yellow-1">{{ __('View') }}</p></a>
                            </div>
                        </div>
                        <div class="flex flex-col leading-2 md:w-auto w-32">
                            <a class="leading-4 md:mt-0 mt-1" href="{{ route('site.itemDetails', ['code' => $item->item_code, 'name' => cleanedUrl($item->name)]) }}">
                                <p class="text-sm text-gray-10 mt-13p roboto-medium hidden md:block">{{ optional(optional($item->itemCategory)->category)->name }}</p>
                                <p class="text-xs md:text-base text-gray-12 dm-sans md:block hidden">{{ trimWords($item->name, 50) }} </p>
                                <p class="text-xs md:text-base text-gray-12 dm-regular md:dm-sans md:font-medium md:hidden">{{ trimWords($item->name, 30) }} </p>
                            </a>

                            <div class="flex leading-6">
                                @if ($item->isDiscountable())
                                <p class="text-xl text-gray-12 dm-sans">{{  formatNumber($item->discounted_price) }}</p>
                                @else
                                    <p class="text-13 md:text-xl text-gray-12 dm-bold"> {{ formatNumber($item->price) }} </p>
                                @endif
                            </div>
                                    ...
                                    <ul class="flex space-x-1">
                                        @for ($i = 1; $i <= 5; $i++)
                                            @if (round($item->review_average) >= $i)
                                                <li class="mt-1">
                                                    <svg class="text-yellow-1 h-13p w-13p md:h-auto md:w-auto" width="17" height="16" viewBox="0 0 17 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
