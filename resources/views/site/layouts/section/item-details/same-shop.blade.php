<div class="mt-15p md:mt-30p">
    <p class="font-medium dm-sans text-base md:text-17 text-gray-12">{{ __('More Items From Them') }}</p>
    <div class="mt-2 md:mt-3 lg:mt-3 lg:flex-col flex overflow-auto gap-5">
                 ...
                 <div class="flex space-x-3.5 md:space-x-5 md:mb-5">
                    <div class="md:w-1/3 md:h-24">
                        <div class="relative z-ng md:w-auto md:h-auto rounded">
                            <div class="flex justify-center p-5 bg-gray-11 w-84p h-84p md:w-24 md:h-24 rounded" >
                                <img class="w-full object-contain" src="{{ $item->fileUrl() }}" alt="">
                            </div>
                            ...
                            <div class="opacity-0 hover:opacity-100 duration-300 absolute inset-0 z-10 flex justify-center items-center text-xl text-white font-semibold add-to-cart" data-itemId={{ $item->id }}>
                                <div class="absolute flex justify-center h-6 w-6 cursor-pointer bottom-2 right-2">
                                    <a href="" class="relative block w-fill">
                                        <div slot="icon" class="relative">
                                            <div class="h-6 w-6 p-1 text-gray-12 hover:bg-yellow-1 border border-gray-2 rounded-full bg-white">
                    ...
                    <div class="md:w-2/3 w-28 flex items-center">
                        <div class="item-rating">
                            ...
                            <div class="-mt-1">
                                ...
                                <p class="text-sm text-gray-10 dm-sans mt-0.5">{{  formatNumber($item->discounted_price) }}</p>
                                @else
                                    <p class="text-xs md:text-sm text-gray-10 dm-sans mt-0.5">{{  formatNumber($item->price) }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
