    ...
    <div class="flex items-center mb-6 justify-center space-x-1">
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <a href="#"
                            class="px-2 bg-yellow-1 h-8 w-8 text-center py-1 rounded-sm text-gray-10 text-15  roboto-medium font-medium  hover:text-gray-12 transition ease-in-out delay-120 hover:bg-yellow-1">
                            {{ $page }}
                        </a>
                    @else
                        <a href="{{ $url }}"
        ...
        @if ($paginator->hasMorePages())
            <a href="{{ $paginator->nextPageUrl() }}"
                class=" w-120 inline-flex  checkOut process-hover relative items-center px-2  transition ease-in-out delay-120 text-gray-10 text-center hover:text-gray-12 roboto-medium font-medium text-sm  ">
                <span class=" dm-sans font-medium -ml-0.5">{{ __('Next') }}</span>
                <svg class="hover:text-gray-10 ml-10 absolute" width="11" height="7" viewBox="0 0 11 7" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd"
        ...
        @else
            <a href="" class=" text-sm dm-sans font-medium flex text-gray-10 w-44 ">
