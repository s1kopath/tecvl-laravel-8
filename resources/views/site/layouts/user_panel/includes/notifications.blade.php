@foreach (['success', 'danger', 'fail', 'warning', 'info'] as $msg)
    @if ($message = Session::get($msg))
        <div class="absolute hide-div right-72">
            <div class="{{ $msg == 'fail' ? ' border-reds-1' : ' border-green-1' }} mt-10 w-417 dm-sans font-medium text-gray-12 text-lg pl-30p py-30p border-b-4 rounded hidden xl:block"
        ...
        <div class="my-auto block xl:hidden" x-data="{ showModal1: true }" :class="{ 'overflow-y-hidden': showModal1 }">
            <!-- Modal1 -->
            <div class="fixed inset-0 bg-black bg-opacity-50 pt-60 duration-300 overflow-y-auto" x-show="showModal1"
...
@endforeach
@if ($errors->any())
<div class="absolute hide-div right-0">
    @foreach ($errors->all() as $error)
        <div class="{{ $msg == 'fail' ? '' : 'border-reds-1 ' }}w-417 dm-sans font-medium text-gray-12 text-lg  pl-30p py-30p border-b-4 rounded hidden xl:block mt-10"
            role="alert" x-data="{ open: true }" x-show.transition="open">
            <div class="flex justify-between">
                <div class="flex">
                    <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 pt-60 duration-300 overflow-y-auto" x-show="showModal1"
        ...
        <div class="relative sm:w-1/2 sm:mx-auto mx-5 my-10 opacity-100">
            <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-50"
                ...
                x-transition:enter-end="scale-100" x-transition:leave="transition transform duration-300"
                x-transition:leave-start="scale-100" x-transition:leave-end="scale-0">
                <div class="{{ $msg == 'fail' ? ' ' : 'border-reds-1 ' }} bg-white mx-auto dm-sans font-medium text-gray-12 text-sm  mt-20 px-30p py-30p border-b-4 rounded"
                    role="alert" x-data="{ open: true }" x-show.transition="open">
                    <div class="flex">
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                            viewBox="0 0 32 32" fill="none">
