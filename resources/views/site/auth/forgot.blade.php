    ...
    <div class="lg:flex" id="login-container">
        ...
        <div class="lg:w-1/2 xl:max-w-screen-sm">
            <div class="mt-24 px-12 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
                  ...
                  xl:text-bold text-green-500">{{ __('Forgot Password') }}</h2>
                <div class="mt-12">
                    <form method="post" action="{{ route('site.login.sendResetLink') }}">
                        @csrf
                        <div>
                            <div class="text-md font-bold text-gray-700 tracking-wide">{{ __('Email') }}</div>
                            <input name="email" class="w-full text-sm py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" required type="email" placeholder="{{ __('Enter your email address') }}" value="{{ old('email') }}">
                            @if ($errors->any())
