    ...
    <div class="lg:flex" id="login-container">
        ...
        <div class="lg:w-1/2 xl:max-w-screen-sm">
            <div class="mt-24 px-12 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
                  ...
                  xl:text-bold text-green-500">{{ __('Verify Email') }}</h2>
                <div class="mt-12">
                    <form method="get" action="{{ route('site.password.reset', ['token' => 'otp']) }}">
                        @csrf
                        <div>
                            <div class="text-md font-bold text-gray-700 tracking-wide">{{ __('OTP') }}</div>
                            <input name="token" class="w-full text-sm py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" required type="text" placeholder="{{ __('Enter your OTP') }}" value="">
                            @if ($errors->any())
