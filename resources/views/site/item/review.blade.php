...
<div class="text-center border-b py-2 md:py-0 md:pb-2">
    <h4> {!! __('To give a review, you need to :x', ['x' => '<button class="underline text-green-500 open-login-modal">'. __('Login') .'</button>']) !!} </h4>
</div>
@endif
@forelse($reviews as $review)
<section class="text-gray-600 body-font overflow-hidden">
        <div class="container">
            <div class="md:py-8 mt-5 flex">
                ...
                <div class="flex-grow rtl-direction-space-review">
                    <p class="text-gray-12 text-13 md:text-lg dm-bold">{{ $review->user->name ?? __('Unknown') }}</p>
                    <p class="roboto-medium text-gray-10 text-xs">
                        {{ str_replace('|', 'at', date('d F, Y | g:i a', strtotime($review->format_created_at))) }}
                    </p>
                    <div class="flex justify-between items-center">
                        <div>
                            @if ($review->verifiedUser($review->user_id, $review->item->id))
                                <div class="text-green-500 mt-2 text-xs rounded-full flex">
                            ...
                            <ul class="flex mt-2 md:mt-0">
                                @for($i = 0; $i < 5 ; $i++)
                                    <li>
                                        <svg class="text-green-500 md:mt-25p w-13p h-3 md:h-auto md:w-auto" width="20" height="19" viewBox="0 0 20 19" fill="{{ $i < $review->rating ? " currentColor" : "#ccc" }}" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div x-data="{ showModalEdit: false,
                            ...
                            <div class="edit-modal fixed z-50 scrollbar-w-2 inset-0 w-full h-full bg-black bg-opacity-50 duration-300 overflow-y-auto"
                                ...
                                <div class="relative sm:w-3/4 md:w-1/2 lg:w-2/3 mx-2 sm:mx-auto top-56 opacity-100">
                                    <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-50 p-6 dark:bg-gray-800 dark:text-white"
                                        ...
                                        <header class="flex items-center justify-between p-2">
                                            <div class="flex">
                                                <div class="flex flex-col justify-center bg-green-100 items-center mt-1 h-10 w-10 rounded-full dark:text-gray-2">
                                        ...
                                        <main id="modal-container" class="mt-2">
                                            <section class="text-gray-600 body-font mt-4 ">
                                                <form method="post" action="" id="reviewUpdateFrom" enctype="multipart/form-data">
                                                    ...
                                                    <button type="submit"
                                                    x-on:submit="setTimeout(() => showModalEdit = false, 2000)"
                                                    class="text-white bg-indigo-500 mt-3 border-0 focus:outline-none hover:color_switch_bac rounded text-sm p-2 color_switch_bac color_switch_hover">{{ __('Submit') }}</button>
                                                    <button type="button" class="review-close-modal-btn text-white bg-red-500 mt-3 border-0 focus:outline-none hover:color_switch_bac rounded text-sm p-2 color_switch_bac color_switch_hover"
                    ...
                    @if ($review->objectFile()->get()->isNotEmpty())
                        <div class="flex mt-4">
                            @foreach ($review->filesUrlold() as $key => $file)
                            <div class=" w-11 h-11 flex items-center border mr-7p">
                                <img data-src="{{ $file }}" id="review-image-{{ $key }}" class="image-thumbnail cursor-pointer object-contain w-full h-full" src="{{ $file }}" alt="">
                            </div>
                            @endforeach
                        </div>
                    @endif
...
<div class="text-center roboto-medium text-sm py-4">
    ...
    @if (!auth()->user())
        <h4> {!! __('To give a review, you need to :x', ['x' => '<button class="underline text-green-500 open-login-modal">'. __('Login') .'</button>']) !!} </h4>
    @elseif (optional(auth()->user()->review->where('item_id', $item->id ?? 0)->first())->is_public === 0)
        <h4 class="text-green-500">{{ __('Your review is not publish yet.') }}</h4>
    @else
        <h4>{{ __('Let others know what do you think and be the first to write a review.') }}</h4>
    @endif
</div>
@endforelse
<div id="review-paginate" style="z-index: -1">
