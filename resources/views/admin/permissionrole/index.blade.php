...
@section('css')
  <link rel="stylesheet" href="{{ asset('public/dist/css/role-permission.css') }}">
@endsection
@section('content')
...
<div class="col-sm-12 list-container" id="permission-list-container">
    <div class="card">
        <div class="card-header">
            <h5> <a href="{{ route('roles.index') }}">{{ __('Permission Roles') }}</a> </h5>
            <span class="d-block text-warning mt-3 font-bold"><i class="fas fa-exclamation-triangle mr-2"></i>{{ __('Permissions applied here are applicable to access that particular link. Please change it with extreme care.') }}</span>
            <div class="card-header-right d-inline-block">
                @if (in_array('App\Http\Controllers\RoleController@create', $prms))
                    <a href="{{ route('roles.create') }}" class="btn btn-outline-primary custom-btn-small">
                        <span class="fa fa-plus"> &nbsp;</span>{{ __('Create Role') }}
                    </a>
                @endif
                            ...
                            <table class="table text-center myTable">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $type => $allValue)
                                        <tr data-toggle="collapse" data-value="{{ $type . $loop->iteration}}" class="accordion-toggle main-group">
                                            <td class="text-left">
                                                <a href="#!" class="collapsed text-dark text-bold" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo"><i class="fas fa-plus pl-3 arrow-icon"></i> {{ ucwords(str_replace('_api', ' ', $type)) }}</a>
                                        ...
                                        <tr data-toggle="collapse" data-target="#{{ $type . $key}}" class="accordion-toggle sub-group hidden" data-value="{{ $type . $loop->parent->iteration }}">
                                            <td class="text-left search">
                                                <a href="#!" class="collapsed ml-3" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo"><i class="fas fa-plus pl-3 arrow-icon"></i> {{ ucfirst(preg_replace('/(?<!\ )[A-Z]/', ' $0', $key)) }}</a>
                                            </td>
                                            @foreach($roles as $role)
                                            <td></td>
                                            @endforeach
                                        ...
                                        <tr>
                                            <td colspan="12" class="hiddenRow" style="border: none">
