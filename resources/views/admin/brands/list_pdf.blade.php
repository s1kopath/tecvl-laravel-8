...
@section('list-table')
    <table class="list-table">
        ...
        @foreach($brands as $key => $brands)
            <tr>
                <td class="text-center list-td"> {!! wrapIt($brands->name, 10, ['columns' => 3]) !!} </td>
                <td class="text-center list-td"> {!! isset($brands->vendor->name) ? wrapIt($brands->vendor->name, 10, ['columns' => 3]) : null !!} </td>
                <td class="text-center list-td"> {!! wrapIt($brands->description, 10, ['columns' => 3]) !!} </td>
