    ...
    <div class="col-md-12" id="brand-edit-container">
        <div class="card">
            <div class="card-header">
                <h5> <a href="{{ route('brands.index') }}">{{ __('Brands') }} </a> >>{{ __('Edit :x', ['x' => __('Brand')]) }}</h5>
                <div class="card-header-right">
                </div>
            </div>
            <div class="card-block table-border-style">
                <div class="row form-tabs">
                    <form action="{{ route('brands.update', $brands->id) }}" method="post" id="brandAdd" class="form-horizontal col-md-12" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12 tab-content form-edit-con m-t-35" id="myTabContent">
                                ...
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            <div class="col-md-9">
                                                <div class="fixSize user-img-con">
                                                    <a class="cursor_pointer" href='{{ $brands->fileUrl() }}'  data-lightbox="image-1"> <img class="profile-user-img img-responsive fixSize rounded-circle" src='{{ $brands->fileUrl() }}' alt="" class="img-thumbnail img-fluid attachment-styles"></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-8 form-container">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-12">
                                                <input type="text" placeholder="{{ __('Name') }}" class="form-control form-width" id="name" name="name" value="{{ $brands->name }}" required oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            <label for="Status" class="col-md-3 control-label">{{ __('Vendor') }}</label>
                                            <div class="col-md-12">
                                                <select class="form-control select2" name="vendor" id="vendor">
                                                    <option value="">{{ __('Select One') }}</option>
                                                    @foreach($vendors as $vendor)
                                                        <option value="{{ $vendor->id }}" {{ $brands->vendor_id == $vendor->id ? 'selected' : ''}}>{{ $vendor->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group row">
                                            <label for="Status" class="col-md-3 control-label">{{ __('Status') }}</label>
                                            <div class="col-md-12">
                                                <select class="form-control select2" name="status" id="status">
                                                    <option value="Active" {{ $brands->status == "Active" ? 'selected' : ''}}>{{ __('Active') }}</option>
                                                    <option value="Inactive" {{ $brands->status == "Inactive" ? 'selected' : ''}}>{{ __('Inactive') }}</option>
                                                </select>
                                            </div>
                                        </div>

                                        <div class="form-group row">
                                            <label class="col-md-3 control-label">{{ __('Upload Image') }}</label>
                                            <div class="col-md-12">
                                                <div class="custom-file">
                                                    <input type="file" class="custom-file-input form-control form-width" name="image" id="validatedCustomFile" accept="image/*" >
                                        ...
                                        <div class="form-group row" id="divNote">
                                            ...
                                            <div class="col-md-12" id='note_txt_1'>
